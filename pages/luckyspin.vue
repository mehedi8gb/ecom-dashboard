...
<template>
    ...
    <router-link to="/manageShop" class="w-9 h-9 rounded-full overflow-hidden cursor-pointer">
      ...
      <div class="min-h-screen bg-gradient-to-b from-red-800 to-red-950 flex flex-col items-center justify-center p-6 relative">
        ...
        <!-- Sparkles effect - Make sure they cover the whole screen -->
        ...
        <div class="max-w-2xl w-full space-y-8">
          ...
          <div class="flex flex-col justify-center items-center">
            ...
            <div class="w-full max-w-xs mt-4 flex flex-col items-center">
              ...
              <button 
                @click="spin" 
                class="bg-yellow-500 text-black px-4 py-2 rounded disabled:bg-gray-400 hover:bg-yellow-400" 
...
onMounted(() => {
  ...
  // Random spin degrees (between 1800 and 3600 for a smooth effect)
  ...
  setTimeout(() => {
    ...
    // Update results (keep last 3 spins)
    results.value.push(selectedItem.value);
    if (results.value.length > 3) results.value.shift();

    saveResults(); // Save results to localStorage
  }, 3000); // Wait for 3s for animation to complete
};
